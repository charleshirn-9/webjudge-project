...
2.  **White Agent (The Test Subject):** Runs locally or on a server. It uses **Playwright** to perform real web navigation tasks (e.g., "Find a PS5 on Amazon") and returns execution traces. 
...
*   **Real Browser Navigation:** Includes a reference White Agent implementation.
...
**What happens next?**
1.  The script sends a request to the **Green Agent** (on Render).
2.  The Green Agent contacts your **White Agent** (via Ngrok).
3.  **Chromium opens on your computer**, performs the search, and closes.
4.  Evidence is sent back to the cloud.
5.  Gemini grades the attempt.
6.  The final Score & Verdict are printed in your terminal.
...
*   `playwright_white_agent_api.py`: The **White Agent** server. Runs Playwright to execute tasks. Naive White Agent, that print the entire query in your browser and click on the first link.
